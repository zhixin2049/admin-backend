...
// ============================================================
// 专栏卡片详情页 — /major/:slug/:cardNum (cardNum = 1-based index)
// 样式 100% 复刻 MajorDetailBig (/major/:slug/big)
// ============================================================
...
const MajorCardDetail: React.FC = () => {
  const { slug, cardNum } = useParams<{ slug: string; cardNum: string }>();
  ...
  // 从 data.columnCards 按 1-based 序号取卡片
  const idx = parseInt(cardNum || '1', 10) - 1;
  const card = data?.columnCards[idx];

  // 获取卡片描述列表
  const allDescriptions: { title: string; content: string; updatedAt?: string; isVisible?: boolean }[] =
    card?.descriptions && card.descriptions.length > 0
      ? card.descriptions
      : card?.content
        ? [{ title: '', content: card.content }]
        : [];
  const descriptions = allDescriptions.filter((d) => d.isVisible !== false);
    ...
    <div style={styles.page}>
      ...
      <div style={styles.breadcrumbWrapper}>
        <Flex align="center" gap="small" style={styles.breadcrumb}>
          ...
          <Text style={styles.breadcrumbSep}>/</Text>
          <Text style={styles.breadcrumbCurrent}>{card.title}</Text>
        </Flex>
      </div>
      ...
      <div style={styles.main}>
        ...
        <div style={styles.contentArea}>
          ...
          <div id="intro" style={styles.introCard}>
            <h2 style={styles.introCardTitle}>{card.title}</h2>
          </div>
...
export default MajorCardDetail;
